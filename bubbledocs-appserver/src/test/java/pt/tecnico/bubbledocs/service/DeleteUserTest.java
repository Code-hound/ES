...
public class DeleteUserTest extends BubbleDocsServiceTest {
	...
	/*
	 * 
	 * @Test(expected = UnknownBubbleDocsUserException.class) public void
	 * userToDeleteDoesNotExist() { new DeleteUser(root,
	 * USERNAME_DOES_NOT_EXIST).execute(); }
	 * 
	 * @Test(expected = UnauthorizedOperationException.class) public void
	 * notRootUser() { String ars = addUserToSession(USERNAME); new
	 * DeleteUser(ars, USERNAME_TO_DELETE).execute(); }
	 * 
	 * @Test(expected = UserNotInSessionException.class) public void
	 * rootNotInSession() { removeUserFromSession(root);
	 * 
	 * new DeleteUser(root, USERNAME_TO_DELETE).execute(); }
	 * 
	 * @Test(expected = UserNotInSessionException.class) public void
	 * notInSessionAndNotRoot() { String ars = addUserToSession(USERNAME);
	 * removeUserFromSession(ars);
	 * 
	 * new DeleteUser(ars, USERNAME_TO_DELETE).execute(); }
	 * 
	 * @Test(expected = UserNotInSessionException.class) public void
	 * accessUserDoesNotExist() { new DeleteUser(USERNAME_DOES_NOT_EXIST,
	 * USERNAME_TO_DELETE).execute(); }
	 */
}
